...
namespace Encoder
{
	...
	float Characters_To_Float(const char* Representation)
	{
		...
		Bits |= Char_To_Integer(Representation[0]);
		Bits |= Char_To_Integer(Representation[1]) << 6;
		Bits |= Char_To_Integer(Representation[2]) << 12;
		Bits |= Char_To_Integer(Representation[3]) << 18;

		Bits |= Char_To_Integer(Representation[4]) << 24;
		Bits |= Char_To_Integer(Representation[5]) << 28;
